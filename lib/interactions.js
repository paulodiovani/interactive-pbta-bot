...
const interactionToData = async (interaction, locales, { DICE, COLOR }, { getClient }) => {
...
export const waitForInteractions = (locales, { DICE, COLOR }, { getClient = _getClient } = {}) => {
  getClient().ws.on('INTERACTION_CREATE', async (interaction) => {
    const { name, options } = interaction.data
    info('Received interaction', name, JSON.stringify(options))

    const data = await interactionToData(interaction, locales, { DICE, COLOR }, { getClient })
