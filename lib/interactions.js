...
import { SUB_COMMAND, SUB_COMMAND_GROUP, INTEGER, CHANNEL_MESSAGE_WITH_SOURCE, RESULT_COLOR } from './constants.js'
...
const interactionToData = async (interaction, moveList, { DICE }, { getClient }) => {
  ...
  const embed = new MessageEmbed()
    ...
    .addFields(
      { name: application.total.description, value: total },
...
export const waitForInteractions = (moveList, { DICE }, { getClient = _getClient } = {}) => {
  getClient().ws.on('INTERACTION_CREATE', async (interaction) => {
    const { name, options } = interaction.data
    info('Received interaction', name, JSON.stringify(options))

    const data = await interactionToData(interaction, moveList, { DICE }, { getClient })
